...
- **Locking**: `lockPiece` → `merge` (bake piece into `board`) → `clearLines` → `spawn` (promote `next` to `current`, generate new `next`; if the new piece immediately collides, `endGame` fires).
